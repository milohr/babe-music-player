...
public class LyricsManiaFetcher  {
    ...
    public string fetch_lyrics (string title, string album_artist, string artist) {
        ...
        GLib.File page = GLib.File.new_for_uri (url);
        ...
        return load_successful ? parse_lyrics (uintcontent) : "no dio";
    }

    public string parse_url (string artist, string title) {
        return URL_FORMAT.printf (fix_string (title), fix_string (artist));
    }

    public string fix_string (string str) {
        if (str == null)
            return "errorrrrr";
    ...
    public string parse_lyrics (uint8[] uintcontent) {
        ...
        try {
            ...
            lyrics = lyrics + "\n\n" + ("Lyrics fetched from www.lyricsmania.com");
